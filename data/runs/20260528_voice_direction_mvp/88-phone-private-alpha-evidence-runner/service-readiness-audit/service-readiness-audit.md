...
41. Generate `scripts/create-controlled-direction-trial-session.mjs --json` before any 20-per-direction front/back/left/right hardware pass.
42. Add Meta DAT credentials outside source control and rerun `scripts/glasses-integration-preflight.sh --write-evidence`.
43. Replace stub glasses adapters one platform at a time only after preflight blockers close.
44. Keep this audit report with the run artifacts after every phone/glasses/support session.
